...
#include "kvstore.h"    // Hash 的结构体和接口声明
 ...
 * 但 kvs_hash.c 会调用 kvs_malloc 和 kvs_free，
 * 所以在测试程序中提供简单实现。
 */
...
int main(){
    const int rounds =100000000;
     // 创建一个初始值全部为 0 的 Hash 管理结构
    kvs_hash_t hash = {0};

    // 初始化 Hash 的桶数组和节点分配策略
    ...
    for (int i = 0; i < rounds; i++) {
        ...
        if (kvs_hash_set(&hash, key, value) != 0) {
            fprintf(stderr, "hash set failed at round %d\n", i);
            kvs_hash_destory(&hash);
            return 1;
        }
        ...
        if (kvs_hash_del(&hash, key) != 0) {
            fprintf(stderr, "hash delete failed at round %d\n", i);
            kvs_hash_destory(&hash);
            return 1;
        }
    }
