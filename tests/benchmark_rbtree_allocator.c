//不经过网络 反复测试 内存池和 malloc 在 增加和删除一个 tree 节点时的性能差异。
#include <stdio.h>      // printf
#include <stdlib.h>     // malloc、free
#include <time.h>       // clock_gettime

#include "kvstore.h"    // tree 的结构体和接口声明
...
int main(){
    const int rounds =100000000;
     // 创建一个初始值全部为 0 的 tree 管理结构
    kvs_rbtree_t tree = {0};

    // 初始化 tree 的桶数组和节点分配策略
    ...
    for (int i = 0; i < rounds; i++) {
        ...
        if (kvs_rbtree_set(&tree, key, value) != 0) {
            fprintf(stderr, "tree set failed at round %d\n", i);
            kvs_rbtree_destory(&tree);
            return 1;
        }
        ...
        if (kvs_rbtree_del(&tree, key) != 0) {
            fprintf(stderr, "tree delete failed at round %d\n", i);
            kvs_rbtree_destory(&tree);
            return 1;
        }
    }
